...
// Methods
const formatPlayer = require(`./format-player2.js`);
const getJSON = require('./get-json.js');
const formatSeason = require(`./format-season.js`);
	...
	async execute(variables) {
        ...
        async function getPlayerStats(update, season, currentInteraction) {
            ...
            if (image) {
                ...
                embed.addField(`PPG`, `\`${d ? p.ppg : p.pts}\``, true);
                embed.addField(`RPG`, `\`${d ? p.rpg : p.reb}\``, true);
                embed.addField(`APG`, `\`${d ? p.apg : p.ast}\``, true);
                embed.addField(`SPG`, `\`${d ? p.spg : p.stl}\``, true);
                embed.addField(`BPG`, `\`${d ? p.bpg : p.blk}\``, true);
                embed.addField(`PF`,  `\`${d ? (p.pFouls).toFixed(1) : p.pf}\``, true);
                embed.addField(`TOPG`,`\`${d ? (p.turnovers).toFixed(1) : p.turnover}\``, true);
                embed.addField(`MPG`, `\`${(d ? p.mpg : p.min)}\``, true);
                embed.addField(`TOV%`, `\`${_tovp}\``, true);
                embed.addField(`FG%`, `\`${d ? p.fgp : (p.fg_pct * 100).toFixed(1)}\``, true);
                embed.addField(`3P%`, `\`${d ? p.tpp : (p.fg3_pct * 100).toFixed(1)}\``, true);
                embed.addField(`FT%`, `\`${d ? p.ftp : (p.ft_pct * 100).toFixed(1)}\``, true);
                embed.addField(`2P%`, `\`${_2pp}\``, true);
                embed.addField(`EFG%`, `\`${_efgp}\``, true);
                embed.addField(`TS%`, `\`${_tsp}\``, true);
                embed.addField(`GP`, `\`${p.g}\``, true);
                if (d) embed.addField(`GS`, `\`${p.gamesStarted}\``, true);
        ...
        collector.on(`collect`, async i => {
            ...
            await require(`./add-to-button-count.js`)(con);
